				  ...
				  switch ($key) {
					  ...
					  case "image":
					  	if ( isset($_POST[$keycode . $key]) && $_POST[$keycode . $key] != "") {
								if (preg_match('#^http:\/\/#i', substr($_POST[$keycode . $key],0, 8)) && preg_match('#^(.*)\.(png|gif|jpg|jpeg)$#i', $_POST[$keycode . $key]) ) {		
...
if( gxtb_fb_lB_debug ) {
	...
	## [OpenGraph][on]-Option seperat abfragen #
	$GBLikeButton['OpenGraph']['w3c'] = ( isset($_POST['opengraph_w3c'])) ? $_POST['opengraph_w3c']:0;
