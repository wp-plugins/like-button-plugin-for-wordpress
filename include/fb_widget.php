...
?><iframe src="http://www.facebook.com/plugins/like.php?href=<?php echo urlencode($permalink); ?>&layout=<?php echo $gxtb_fb_lB_data['layout']; ?>&amp;show_faces=<?php echo $gxtb_fb_lB_data['faces']; ?>&amp;width=<?php echo $width; ?>&amp;action=<?php echo $gxtb_fb_lB_data['verb']; ?>&amp;<?php echo $font; ?>colorscheme=<?php echo $gxtb_fb_lB_data['color']; ?><?php echo $ref ?>" scrolling="<?php echo $scrolling; ?>" frameborder="<?php echo $frameborder; ?>" allowTransparency="<?php echo $trans; ?>" style="border:<?php echo $borderstyle; ?>; overflow:<?php echo $gxtb_fb_lB_data['overflow']; ?>; width:<?php echo $width; ?>px; height:<?php echo $gxtb_fb_lB_data['height']; ?>px"></iframe>
<?php if( isset( $gxtb_fb_lB_data['css'] ) && !empty( $gxtb_fb_lB_data['css'] ) ) {
			echo '</div>';
		}
?>
...
?>
    ...
    <p><label><?php _e('Title', 'gb_like_button'); ?> <?php _e('<b>(required)</b>', 'gb_like_button'); ?><br />
    <input name="gxtb_fb_lB_widget_title" type="text" value="<?php echo $gxtb_fb_lB_data['title']; ?>" />
    </label></p>

    <p><label><?php _e('URL', 'gb_like_button'); ?> <?php _e('<b>(required)</b>', 'gb_like_button'); ?><br />
...
<input name="gxtb_fb_lB_widget_dynamic" type="checkbox" class="checkbox" <?php if ($gxtb_fb_lB_data['dynamic']) echo("checked"); ?>/>  <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER ?>/images/rot17a.gif"  onmouseover="tooltip.show('<u><?php _e('Activated', 'gb_like_button'); ?>:</u> <?php _e('Every Post/Page has its own Like-Button. Which means for every page on your side there will be a unique Like-Button.', 'gb_like_button'); ?> <?php _e('(recommended)', 'gb_like_button'); ?><br /><u><?php _e('Deactivated', 'gb_like_button'); ?>:</u> <?php _e('Every Post/Page has the same Like-Button. Which means if you click on it, it looks like you like/recommend every post even if you have not read it before.', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();">
    ...
    ?>
      ...
      <p><label><?php _e('Font', 'gb_like_button'); ?><br />
		 ...
		 <p><label><?php _e('CSS-Design', 'gb_like_button'); ?><br />
...
function gxtb_fb_lB_widget_sidebar_init()
{
	...
	if ( version_compare( $wp_version, '2.8', '>=' ) ) {
		...
		wp_register_sidebar_widget(__('Facebook-Like-Button-Generator', 'gb_like_button'), __('Facebook-Like-Button-Generator', 'gb_like_button'), array( $this, 'gxtb_fb_lB_widget' ) );
	...
	// out of use - currently (since v3.0)
