...
function gxtb_fb_lB_WidgetClass() {
	...
	# new Options currently only available if the v4.4.3.1 is installed and the GBWidgetCleaner is available to run#
	if ( version_compare( gxtb_fb_lB_version, '4.4.3.1', '>=' ) ) {
		include('gb_cleaner.php');
		$GBLikeButtonWidgetCleaner = new GBLikeButtonWidgetCleaner();
		$GBLikeButtonWidgetCleaner->WidgetResetAndAdd();
	}
...
';
echo '
<link rel="stylesheet" type="text/css" href="' . gxtb_fb_lB_PLUGIN_FOLDER . 'admin/css/widget.css" />
';
echo '
...
?><iframe src="http://www.facebook.com/plugins/like.php?href=<?php echo urlencode($permalink); ?>&layout=<?php echo $this->GBLikeButtonWidget['LikeButton']['layout']; ?>&amp;show_faces=<?php echo $this->GBLikeButtonWidget['LikeButton']['faces']; ?>&amp;width=<?php echo $width; ?>&amp;action=<?php echo $this->GBLikeButtonWidget['LikeButton']['verb']; ?>&amp;<?php echo $font; ?>colorscheme=<?php echo $this->GBLikeButtonWidget['LikeButton']['color']; ?><?php echo "&amp;height=" . $height; ?><?php echo $ref; ?>" scrolling="<?php echo $scrolling; ?>" frameborder="<?php echo $frameborder; ?>" allowTransparency="<?php echo $trans; ?>" style="border:<?php echo $borderstyle; ?>; overflow:<?php echo $this->GBLikeButtonWidget['LikeButton']['overflow']; ?>; width:<?php echo $width; ?>px; height:<?php echo $height; ?>px"></iframe>
<?php if( isset( $this->GBLikeButtonWidget['LikeButton']['css'] ) && !empty( $this->GBLikeButtonWidget['LikeButton']['css'] ) ) {
			echo '</div>';
		}
?>
...
<input name="gxtb_fb_lB_widget_dynamic" type="checkbox" class="checkbox" <?php if ($this->GBLikeButtonWidget['LikeButton']['dynamic']) echo("checked"); ?>/>  <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER; ?>/images/rot17a.gif"  onmouseover="tooltip.show('<u><?php _e('Activated', 'gb_like_button'); ?>:</u> <?php _e('Every Post/Page has its own Like-Button. Which means for every page on your side there will be a unique Like-Button.', 'gb_like_button'); ?> <?php _e('(recommended)', 'gb_like_button'); ?><br /><u><?php _e('Deactivated', 'gb_like_button'); ?>:</u> <?php _e('Every Post/Page has the same Like-Button. Which means if you click on it, it looks like you like/recommend every post even if you have not read it before.', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();">
    ...
    ?>
      ...
      <p><label><?php _e('Font', 'gb_like_button'); ?><br />
		...
		<p><label><?php _e('Facebook-Insights (REF-Attribute)', 'gb_like_button'); ?> <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER; ?>/images/rot17a.gif"  onmouseover="tooltip.show('<?php _e('This new Attribute will give you some new kind of information about the effectiveness of your like-button. Read more in the Plugin-FAQ or on the official Facebook Insights page.', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();"><br />
   ...
   if (isset($_POST['gxtb_fb_lB_widget_url']) || isset($_POST['gxtb_fb_lB_widget_title']) ){
	   ...
	   foreach ($this->GBLikeButtonWidget[$area] as $key => $value) { 
		...
		switch($key) {
			...
			case "scrolling":
			case "trans":
				   	$this->GBLikeButtonWidget[$area][$key] = ( isset($_POST[$keycode . $key]) ) ? attribute_escape($_POST[$keycode . $key]):0;
			break;
			
			default:
				   	$this->GBLikeButtonWidget[$area][$key] = ( isset($_POST[$keycode . $key]) ) ? attribute_escape($_POST[$keycode . $key]):"";
			break;	
		}
	   }	   
...
<div class="gb-widget">
    ...
    <p><label><?php _e('Border-Style', 'gb_like_button'); ?> <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER ?>/images/rot17a.gif"  onmouseover="tooltip.show('<?php _e('For Example: none or solid - Please do not enter the ;-symbol. This will be added automatically. But if you add two attributes you have to seperate the first two but do not enter it at the end of this statemente.', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();"><br />
	...
	<p><label><?php _e('CSS-Design', 'gb_like_button'); ?> <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER ?>/images/rot17a.gif"  onmouseover="tooltip.show('<?php _e('If you enter something into this breaks this will work as a style-attribute within the iframe-Output.', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();"><br />
...
<div class="gb-widget">
    ...
    <input name="gxtb_fb_lB_widget_act_filter" type="text" value="<?php echo $this->GBLikeButtonWidget['ActivityFeed']['filter']; ?>" size="auto" maxlength="200"/> <img src="<?php echo gxtb_fb_lB_PLUGIN_FOLDER ?>/images/rot17a.gif" onmouseover="tooltip.show('<?php _e('More Information about this filter on: http://developers.facebook.com/docs/reference/plugins/activity/', 'gb_like_button'); ?>');" onmouseout="tooltip.hide();">
    </label><br />
