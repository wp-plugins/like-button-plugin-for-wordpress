...
/*
+----------------------------------------------------------------+
+	Like-Button-Plugin-For-Wordpress [v4.4.4] - GB-Admin-Bar-Menu [v1.0 FINAL]
...
if(is_single() || is_page() ) {
	...
	$method = (gxtb_fb_lB_debug == true) ? "get":"post";
...
if (version_compare( $wp_version, '2.9', '>=' ) && current_theme_supports('post-thumbnails')) {
	...
	$fbpicture .= '<input size="50" type="text" maxlength="100" value="'.  stripslashes($fbpic) .'" id="gb_adminbar_fbpicture" name="gb_adminbar_fbpicture" class="adminbar-input" onchange="return gb_adminbar(this.value)">';
	$fbpicture .= ' ' . __("Connected Image", gxtb_fb_lB_lang ) . ' ';
    $fbpicture .= '<button type="submit" name=gb_adminbar_picture-submit= class="adminbar-button">
...
$wp_admin_bar->add_menu( array( 'parent' => $id.'settings', 'title' => __( 'General Settings', gxtb_fb_lB_lang ), 'href' => $url.'button', 'meta' => array('target' => '_blank')  ) );
...
function AdminBar_Save($post_id, $wp_version) {
	...
	if ( !current_user_can('activate_plugins') || !is_user_logged_in() ) { return ""; }
		...
		switch ($_POST['fb_like_hidden']) {
			...
			case "fbpicture":
					...
					echo $fbpicture;
					if (isset($fbpicture)) {
						if($fbpicture != "" && !preg_match('#^http:\/\/#', substr($fbpicture,0, 8))) { $fbpicture = "http://" . $fbpicture; }
