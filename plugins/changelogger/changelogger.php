...
## OFFICIAL PLUGIN BY Oliver Schl&ouml;be
## THIS IS A MODIFIED VARIANT OF THE CHANGELOGGER-PLUGIN UNDER THE GPL-LICENCE. THIS IS NOT OUR WORK AND ALL OF THIS CODE WAS WRITTEN AND DISTRIBUTED BY THE OFFICIAL AUTHOR!
## Plugin-Download: http://wordpress.org/extend/plugins/changelogger/

#########################################################
### THIS IS NOT THE WORK OF GB-World.net and its Team ###
#########################################################
...
if ($file == "like-button-plugin-for-wordpress/gb_fb-like-button.php") {
				...
				if( isset($api->sections['changelog']) ) {
					...
					if( $section_exists ) {
						...
						if ( isset($api->tested) && version_compare($api->tested, $cur_wp_version, '>=') ) {
							$output .= ' ' . sprintf(__('Compatibility with WordPress %1$s: 100%% (according to GB-World.net).', 'changelogger'), $cur_wp_version);
