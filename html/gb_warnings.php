...
#####################################################################################
	...
	$gxtb_fb_lB_meta = get_option('gxtb_fb_lB_meta');
		...
		if ( !is_numeric($gxtb_fb_lB_meta['admins']) ){
			$gxtb_fb_lB_warning["Open Graph Protocol - Meta Tags"] = __('You did not enter a valid Admin-ID. Please visit <a href="http://apps.facebook.com/whatismyid/" target="_blank">this site</a> to get your Facebook-ID (which is used as Admin-ID).', gxtb_fb_lB_lang);
			$gxtb_fb_lB_warning['warning'] += 1;
		} else {
			unset($gxtb_fb_lB_warning["Open Graph Protocol - Meta Tags"]);
			$gxtb_fb_lB_warning['warning'] -= 1;
		 }
...
#####################################################################################
	...
	global $gxtb_fb_lB_MaxWarnings;
	$gxtb_fb_lB_MaxWarnings = 6 + $gxtb_fb_lB_WidgetWarningNr;
...
public static function gxtb_fb_warnings_output() {
	...
	if( $gxtb_fb_lB_count > 0 ) {
			...
			echo "<div id='message' class='error'><p>";
				...
				echo "<strong>";
				echo sprintf( '<a href="options-general.php?page=%s">%s</a>', gxtb_fb_lB_page . "#reldefinitions" , __('FB-Like Button', gxtb_fb_lB_lang) );
				echo "</strong>";
