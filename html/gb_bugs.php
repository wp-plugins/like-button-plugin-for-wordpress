...
## BUG-BOX
public static function gxtb_fb_lB_BugBox() {
	
	echo sprintf( '%s <br><br>%s <a href="options-general.php?page=%s">%s</a>!', 
...
<ul class="bugtracker">
	...
	<?php if ( function_exists( 'fetch_feed' ) ) { 
	    ...
	    if ( $max_items == 0 ) {
			echo '<li class="ajax-error">Currently there are no reported bugs.</li>';
	    } else {
		    // Loop through each feed item and display each item as a hyperlink.
		    foreach ( $rss_items as $item ) { ?>
		    <li>
				<a target="_blank" class="gxtb_feed" href='<?php echo $item->get_permalink(); ?>' title='<?php echo 'Posted '.$item->get_date('j F Y | g:i a'); ?>'><?php echo $item->get_title(); ?></a>
		    </li><?php 
			} 
		}
    } else { 
