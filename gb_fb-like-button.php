...
if ( !defined( $gxt_fb_like_button_shortcode . '_version' ) )
	define( $gxt_fb_like_button_shortcode . '_version', "4.3.1" );
...
function gxtb_fb_lB_activate(){
	...
	## Generate PLUGIN-OPTIONS ## - deprecated until v5.0##
	...
	if(!get_option('gxtb_fb_lB_design')) {
		...
		'css' => $settings['css'],
		'cssbox' => $settings['cssbox'],
		'br_before' => $settings['br_before'],
	...
	$gxtb_fb_lB_generator = array(
		...
		$settings = get_option('gxtb_fb_lB_settings');
		$settings['language'] = $generator['language'];
		$settings['dynamic'] = $generator['dynamic'];
